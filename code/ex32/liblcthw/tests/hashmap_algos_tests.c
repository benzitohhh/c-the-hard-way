...
int gen_keys(DArray * keys, int num_keys)
{
    ...
    //check(urand != NULL, "Failed to open /dev/urandom"); // commented out to prevent compiler warning

    int rc = 0;

    struct bStream *stream = bsopen((bNread) fread, urand);
    //check(stream != NULL, "Failed to open /dev/urandom");  // commented out to prevent compiler warning
